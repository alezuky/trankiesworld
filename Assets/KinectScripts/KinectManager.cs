...
public class KinectManager : MonoBehaviour 
{
	...
	public bool player1added = false;
	...
	// Calibration pose player 1
	//public KinectGestures.Gestures playerCalibrationPose;

	// Calibration pose player 2
	//public KinectGestures.Gestures playerCalibrationPose2;
	...
	public Dictionary<Int64, int> dictUserIdToIndex;
	...
	void Update() 
	{
				...
				foreach(KinectGestures.GestureData gestureData in gesturesData)
				{
					if(gestureData.complete)
					{
						...
						foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							...
							{
								listener.Requestforpause (userId, 0);
					...
					else if(gestureData.progress >= 0.1f)
					{
						...
						foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							if (gameObject.GetComponent<Pause>().isPaused != true){
		...
		/*if (player1added == false || player2added == false)
		{
			NotifyUsers();
		}
	*/
	...
	private void ProcessBodyFrameData()
	{
		...
		if((addedUsers.Count > 0) && (player1tracked == false || player2tracked == false))
		{
	...
	void CalibrateUser(Int64 userId, int bodyIndex)
	{
		if (splash == false) {
			...
			if (!alUserIds.Contains (userId)) {
				if (player1tracked == false || player2tracked == false) {

					if (GameObject.Find ("Player_1").GetComponent<AddsPlayer> ().CheckForCalibrationPose (userId, bodyIndex) || GameObject.Find ("Player_2").GetComponent<AddsPlayer> ().CheckForCalibrationPose (userId, bodyIndex)) {
	...
	// notify the gesture listeners about the new users
	/*void NotifyUsers ()	{

		foreach (Int64 userId in alUserIds) {
			foreach (KinectGestures.GestureListenerInterface listener in gestureListeners) {
				string player = listener.UserDetected (userId, 0);
				if (String.Compare(player, "Player_1") == 0) player1added = true;
				if (String.Compare(player, "Player_2") == 0) player2added = true;
			}
		}
	}*/

	// Returns the relative postion of the user and 0 if there is only one user
	//This method assumes there are 2 users at most
	public int GetRelativePostion (Int64 user)
	{
		float position = GetUserPosition (user).x;
		foreach (Int64 userId in alUserIds) {
			position = position - GetUserPosition (userId).x;
		}
		if (position < -0.1)
			return -1; // User in the left
		else if (position > 0.1)
			return -1; // User in the right
		else
			return 0; // Only 1 user
	}
	...
	void RemoveUser(Int64 userId)
	{
		...
		if (userId.CompareTo ( GameObject.Find ("Player_1").GetComponent<GestureListener> ().user) == 0) {
			player1tracked = false;
			Debug.Log ("losing player 1");
			//gameObject.GetComponent<Pause>().PauseGame(1);

		} else if (userId.CompareTo ( GameObject.Find ("Player_2").GetComponent<GestureListener> ().user) == 0) {
			player2tracked = false;
			Debug.Log ("losing player 2");
			//gameObject.GetComponent<Pause>().PauseGame(2);
		}
	...
	public void CheckForGestures(Int64 UserId)
	{
	...
	public bool CheckForCalibrationPose(Int64 UserId, int bodyIndex, KinectGestures.Gestures calibrationGesture)
	{
